...
import com.escalatesoft.crs.{CRSDef, MeterCRSDef}
...
case class Polygon[CRS: CRSDef](coords: Vector[Coord2D[CRS]]) extends GeometryType[CRS] {
  ...
  def transformCRS[NEW_CRS: CRSDef]: Polygon[NEW_CRS] = {
...
object Polygon {
  def apply[CRS: CRSDef](coords: (Double, Double)*): Polygon[CRS] = {
    val gCoords = coords.toVector.map { case (x, y) => Coord2D[CRS](x, y) }
    new Polygon[CRS](gCoords)
  }

  def area[CRS: CRSDef](poly: Polygon[CRS]): Double = {
    poly.area
  }

  def areaSquareMeters[CRS: MeterCRSDef](poly: Polygon[CRS]): Double = {
    poly.area
  }
}
