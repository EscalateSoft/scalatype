...
case class Feature[CRS: CRSDef, GEOM <: GeometryType[CRS], +T](geometry: GEOM, attributes: T) {
  def mapAttributes[U](transform: T => U): Feature[CRS, GEOM, U] = {
    Feature[CRS, GEOM, U](geometry, transform(attributes))
  }

  private def addAttrInner[TMT, TM <: TMap[TMT], ATTR: TypeTag](attr: ATTR)(implicit ev: T <:< TM): Feature[CRS, GEOM, TMap[TMT with ATTR]] = {
...
object Feature {
  implicit class RichFeature[CRS: CRSDef, GEOM <: GeometryType[CRS], T](feature: Feature[CRS, GEOM, TMap[T]]) {
